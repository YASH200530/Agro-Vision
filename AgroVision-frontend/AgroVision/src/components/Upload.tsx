...
import { Upload as UploadIcon, Camera, X, Loader2, CheckCircle, AlertCircle, Share2 } from "lucide-react";
...
export default function Upload() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const res = await fetch(`${base}/api/ai/analyze-image`, { method: "POST", body: formData });
      const data = await res.json();
      if (res.ok && data.ok && data.analysis) {
        setResult({
          disease: data.analysis.diagnosis,
          confidence: data.analysis.confidence,
          treatment: data.analysis.treatment,
        });
      } else {
        alert(data.message || "AI analysis failed.");
        setResult({ disease: "Analysis Failed", confidence: 0, treatment: "Try a clearer image." });
      }
    } catch (e) {
      alert("Network/server error.");
      setResult({ disease: "Connection Error", confidence: 0, treatment: "Server unreachable." });
    } finally {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {!result ? (
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-2xl border-4 border-dashed border-gray-200 dark:border-gray-700 transition-all duration-300">
            {imagePreview ? (
              <div className="relative">
                <img src={imagePreview} alt="Selected Leaf" className="w-full h-80 object-cover rounded-2xl shadow-lg mb-6" />
                <button
                ...
                <div className="flex justify-center">
                  ...
                  >
                    {isAnalyzing ? (<><Loader2 className="w-5 h-5 mr-3 animate-spin" /> {t("upload.analyzing")}</>) : t("upload.analyze_button")}
                  </button>
                </div>
              </div>
            ) : (
              <div
                ...
                className={`flex flex-col items-center justify-center p-16 text-center rounded-2xl border-4 border-dashed cursor-pointer transition-all duration-300 
                  ${dragOver ? "border-agri-500 bg-agri-50/50" : "border-gray-300 dark:border-gray-600 hover:border-agri-400 dark:hover:border-agri-400 hover:bg-gray-50 dark:hover:bg-gray-700"}`}
              >
                <UploadIcon className="w-12 h-12 text-agri-600 mb-4" />
                <p className="text-xl font-semibold text-gray-700 dark:text-gray-300 mb-2">{t("upload.drag_drop")}</p>
                <p className="text-lg text-gray-500 mb-4">{t("upload.click_to_browse")}</p>
          ...
          <div className="bg-white/90 dark:bg-gray-800/90 rounded-3xl p-8 shadow-2xl border-4 border-agri-200 dark:border-agri-700">
            <h2 className="text-3xl font-bold text-center mb-6">{t("upload.results.title")}</h2>
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-1/3">
                <img src={imagePreview!} alt="Selected Leaf" className="w-full h-64 object-cover rounded-2xl shadow-lg" />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <StatusIcon className={`w-8 h-8 ${result.disease === "Healthy" ? "text-agri-600" : "text-red-600"}`} />
                    <h3 className="text-3xl font-bold">{result.disease}</h3>
                  </div>
                  <p className={`text-2xl font-bold ${getConfidenceColor(result.confidence)}`}>{result.confidence}%</p>
                </div>
                <div className="border-t pt-4 mt-4">
                  <h4 className="text-xl font-bold text-agri-600 mb-2">{t("upload.results.treatment")}</h4>
                ...
                <div className="mt-8 flex flex-col sm:flex-row justify-center gap-4">
                  ...
                  >
                    <Share2 className="w-5 h-5 mr-3" /> {t("upload.results.share")}
                  </button>
                  <button
                    onClick={() => { setSelectedImage(null); setImagePreview(null); setResult(null); setIsAnalyzing(false); }}
                    className="inline-flex items-center px-8 py-4 border-2 rounded-xl"
                  >
                    <UploadIcon className="w-5 h-5 mr-3" /> {t("upload.results.retry")}
                  </button>
                </div>
              </div>
            </div>
